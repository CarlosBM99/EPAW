...
public class WhoToFollowController extends HttpServlet {
	...
	{
		System.out.println("WhoToFollowController.");
		HttpSession session = request.getSession();
	    try
	    {
	    	int count = 0;
	    	ResultSet result = db.executeSQL("SELECT COUNT(*) FROM users where nickname!='"+ session.getAttribute("nickname") +"';");
	    	if(result.next()){
	    		count += result.getInt(1);
    		}
	    	result = db.executeSQL("SELECT * FROM users where nickname!='"+ session.getAttribute("nickname") +"';");
	    	int i = 0;
	    	BeanUser[] users = new BeanUser[count];
	    	while(result.next()){
	    			users[i] = new BeanUser(result.getString("nickname"),result.getString("first_name"),result.getString("last_name"));
	    			i++;
	    	}
	    	request.setAttribute("users", users);
    		request.setAttribute("users_size", users.length);
    		RequestDispatcher dispatcher = request.getRequestDispatcher("WhoToFollow.jsp");
